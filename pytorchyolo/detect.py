...
def detect_directory(model_path, weights_path, img_path, classes, output_path,
    ...
    """
    img_path에 있는 모든 이미지에 대하여 detecting 실행 후 output_path에 detecting 결과 저장

    model_path: [workspace]/config/yolov3.cfg
    weights_path: [workspace]/weights/yolov3.weights
    img_path: [workspace]/data/samples
    classes: load_classes([workspace]/data/coco.names)
